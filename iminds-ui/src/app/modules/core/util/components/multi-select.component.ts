...
import { stripTags, SelectItem, OptionsBehavior, escapeRegexp } from 'ng2-select';

@Component({
  selector: 'stl-multi-select',
  styleUrls: ['./expression-builder/expression-builder.component.css'],
  providers: [
    {
...
export class MultiSelectComponent implements OnInit, ControlValueAccessor {
  ...
  @Input() public allowCustomInputs: boolean = false;
  ...
  public inputEvent(e: any, isUpMode: boolean = false): void {
    ...
    if (!isUpMode && e.keyCode === 13) {
      ...
      if (this.allowCustomInputs && target.value && target.value.trim().length > 0) {
        let customIteam = new SelectItem(target.value);
        this.selectMatch(customIteam);
      } else if (this.active.indexOf(this.activeOption) === -1) {
  ...
  private selectMatch(value: SelectItem, e: Event = void 0): void {
    ...
    if (this.options.length <= 0 && !this.allowCustomInputs) {
      return;
    }
...
export class Behavior {
  ...
  public actor: MultiSelectComponent;

  public constructor(actor: MultiSelectComponent) {
    this.actor = actor;
  }
